...
local List = require("dogma.lex._.AdvancedList")

--Suite.
return suite("dogma.lex._.AdvancedList", function()
  -----------------
  -- constructor --
  -----------------
  ...
  suite("insert()", function()
    ...
    test("insert(item) - three times", function()
      list:insert("one")
      list:insert("two")
      assert(function() list:insert("three") end):raises("list already full.")
  ...
  suite("remove()", function()
    ...
    test("remove() - three times", function()
      assert(list:remove()):eq("two")
      assert(list:remove()):eq("one")
      assert(function() list:remove() end):raises("empty list, nothing to remove.")
      assert(#list):eq(0)
      assert(list._):has({
        max = 2,
        items = {}
